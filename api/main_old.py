...
class DeveloperMessageConfig:
    ...
    # 共通の開発者メッセージ
    COMMON_DEVELOPER_MESSAGE = """あなたは「動機付け面接」（MI）の専門家として，以下で説明するMIの知識に基づいて，臨床家からの質問に回答します。
...
これらを，以下のスキルを用いることで実現します。これらは頭文字を取ってOARSスキルと呼ばれます。
...
# 出力時に守ること
- この面接は，臨床家とクライエントの間での対話です。あなたは臨床家のフィードバックをする立場です。
...
- いい評価の時も代替案を提示しなさい。
- 短くわかりやすい表現にしなさい。
- 望ましい発言は，あたかもクライエントが，前の発言の続きを述べているような言葉を臨床家が言うことです。当たり前ですが「聞き返し」とはこういう発言です。
（例）クライエント：「働きすぎだと言ってきて，妻をうるさく感じることもあります」
　臨床家：「普段はそんなことないのに。」「仕事の大切さを十分わかってくれないと感じる。」「それには罪悪感もある。」
（例）クライエント：「疲れることが多くて。」
臨床家：「色々と気苦労が多いんですね。」「やりたいことはあるのにできない。」「しっかり休みたい」「じゃあ朝はいっそう辛い」
（例）クライエント：「大事なのはわかるけど難しいって」
臨床家：「今はできない気がしている」「大事なのはもうわかってる，問題はできるかだと。」「もっと現実的な話をしたい」
本当にお願いですから，この種の提案をできるだけ多くしてください。
    """
    
    # 分析特化の追加指示
    ANALYSIS_SPECIFIC_MESSAGE = """
    以下の点に注意してください：
    - 発言の文脈を十分に考慮してください
    ...
    @staticmethod
    def update_common_message(new_message: str) -> None:
        """共通メッセージを更新（必要に応じて）"""
        DeveloperMessageConfig.COMMON_DEVELOPER_MESSAGE = new_message
...
以下の対話文を{PromptManager.get_aspect_description(aspect)}の観点から分析してください。重要な発言を抽出し、以下の項目で評価してください：
"""
...
async def detailed_chat(request: DetailedChatRequest):
    ...
    try:
        ...
        return ChatResponse(
            response=response.output_text or "エラーが発生しました。もう一度お試しください。"
        )
...
async def chat_with_ai(request: ChatRequest):
    ...
    try:
        # 開発者メッセージをシステムメッセージとして追加
        messages_with_dev_msg = [
            {"role": "system", "content": DeveloperMessageConfig.get_base_system_message()}
        ]
        messages_with_dev_msg.extend(request.messages)
        
        response = await client.responses.create(
            model=request.model,
            input=messages_with_dev_msg,
...
async def analyze_cct(text: str, target_behavior: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    return parse_analysis_response(response.output_text)
